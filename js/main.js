...
} else if (window.location.href.includes("/reset") && toast) {
    // give the user rest code in toast message (i don't want to go to sessionStorage)
    toast.children[1].innerHTML = `Check sessionStorage to Get Code -> this is the code ${sessionStorage.getItem('resetCode')}`;
}
...
if (window.location.href.includes("?userAccess")) {
    // to remove ?userAccess params from url
...
if (window.location.href.includes("?updated")) {
    // to remove ?updated params from url
    window.history.replaceState({}, document.title, window.location.pathname);
    toast.classList.add("show");
...
// rePasswordInput for reset password page only
if (rePasswordInput) {
        ...
        if (validEmail(emailInput.value) && validPassword(passwordInput.value)) {
            ...
            if (user && user.email === emailInput.value && user.password === passwordInput.value) {
                ...
                localStorage.setItem(
                    "token",
                    btoa(JSON.stringify({ id: user.id, name: user.name }))
                );
                window.location.href = `${BaseURl}/home.html`;
            } else {
                invalidAccountMsg.classList.add("d-block");
